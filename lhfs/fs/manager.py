...
class FSManager(object):
    ...
    @utils.remotable
    def disk_usage(self):
        usage = objects.DiskUsage(Disk.usage(self.root)).to_dict()
        LOG.info(usage)
        return usage
    ...
    def find(self, partern):
        ...
        for dirPath, name in fs.find(self.root, partern):
            ...
            path_dict = self.get_path_dict('{}/{}'.format(logic_dir, name))
            path_dict['pardir'] = logic_dir
...
class BaseNodeManager(FSManager):
    ...
    def __init__(self, root, node_type, rpc_server=None,
        ...
        self.node = objects.Node(
            node_type, transport=rpc_server and rpc_server.transport or None,
...
class MasterManager(BaseNodeManager):
    ...
    def registry(self, node):
        if node['ip'] in self.nodes:
            LOG.warning('node %s is already exists.', node.get('hostname'))
        else:
            LOG.warning('found new host %s', node.get('hostname'))
        self.nodes[node.get('ip')] = node

    def node_update(self, node):
        self.nodes[node.get('hostname')] = node
        self.nodes[node.get('hostname')]['heartbeat'] = time.time()
        LOG.debug('node update: %s', self.nodes[node.get('hostname')])
    ...
    def get_nodes(self, node_name):
        ...
        if not node_name:
            ...
            nodes = [self.nodes.get(node_name)]
        for node in nodes:
