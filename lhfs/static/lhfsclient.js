class LHFSClient {
    constructor() {
        ...
        this.get = function(url, params){
            if (this.isNodeNull()){ return; }
            console.log(url + `?node=${this.context.node}`)
            this.request('GET', url + `?node=${this.context.node}`, params)
        }
        this.delete = function(url, params){
            if (this.isNodeNull()){ return; }
            this.request('DELETE', url + `?node=${this.context.node}`, params)
        }
        this.post = function(url, params){
            if (this.isNodeNull()){ return; }
            this.request('POST', url + `?node=${this.context.node}`, params)
        }
        this.put = function(url, params){
            if (this.isNodeNull()){ return; }
            this.request('PUT', url + `?node=${this.context.node}`, params)
        }
        ...
        this.getDownloadUrl = function(dirPath){return `/v1/file/${this.context.node}${this._safe_path(dirPath)}`}
        ...
        this.rm = function (path, force=false) {
            let safe_path = this._safe_path(path);
            return axios.delete(`/v1/fs/${this.context.node}${safe_path}`,
                                {params: {force: force}})
        }
        this.mkdir = function (path) {
            let safe_path = this._safe_path(path);
            return axios.post(`/v1/fs/${this.context.node}${safe_path}`)
        };
        this.rename = function(path, new_name){
            let safe_path = this._safe_path(path);
            return axios.put(`/v1/fs/${this.context.node}${safe_path}`,
